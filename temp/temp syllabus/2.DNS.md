...
Considerons notre cas pratique:

**Computer Electronics** (`computerelectronics.be`) est une entreprise internationale d'électronique avec des opérations en Europe et aux États-Unis. Pour gérer efficacement son infrastructure informatique, l'entreprise utilise une structure DNS basée sur :

- **Zones géographiques** pour séparer les opérations par région
- **Zones de service** pour séparer les environnements de développement et de production
...
Notre **espace de noms contient tous les noms de domaine** de notre entreprise.

Mais on peut considerer juste une section de l'espace de noms et on aura une **zone DNS**. 
Une **zone DNS** est une partie de l'espace de noms DNS qui est gérée de manière indépendante. Chaque zone :
- Contient les enregistrements DNS (en détail plus tard) pour un domaine ou sous-domaine spécifique
- Peut être gérée par des serveurs DNS différents 
- Permet une administration décentralisée du réseau

Par exemple, la zone `eu.computerelectronics.be` est être gérée par les serveurs DNS en Europe, tandis que `us.computerelectronics.be` est gérée par ceux aux États-Unis.

Dans notre cas une zone est gérée par un seul serveur DNS.

Le diagramme suivant montre la **structure physique du réseau** avec la répartition géographique et les environnements :

![Diagramme DNS](../diagrams/images/structure_reseau_geographic.png)

Observez maintenant comment cette structure physique se traduit dans l'espace de noms DNS :

![Diagramme DNS](../diagrams/images/fored.png)
