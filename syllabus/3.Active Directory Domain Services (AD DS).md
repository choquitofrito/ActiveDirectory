...
1. **Authentification** (flèche bleue)
   - `pc01-comptabilite` (`192.168.0.101`) demande une authentification à `dns1` (`192.168.0.2`)
...
2. **Accès aux ressources** (flèche jaune)
   ...
   - `dns2` fournit l'adresse IP de `fichiers.ventes` (`192.168.0.20`)
...
### 5.2.1.2. Configuration DNS pour notre environnement de test

#### Infrastructure de test

![Structure du réseau](../diagrams/images/structure_reseau.png)

> **Configuration du laboratoire** :
> - **Contrôleur de domaine** : dns1.computerelectronics.be (192.168.0.2)
> - **Postes clients** :
>   - client1.computerelectronics.be (192.168.0.101)
>   - client2.computerelectronics.be (192.168.0.102)

#### Création des zones DNS

1. Sur le serveur dns1, ouvrez le Gestionnaire DNS :
   ```
   Server Manager -> Tools -> DNS
   ```

2. Création de la zone directe :
   - Clic droit sur 'Forward Lookup Zones' → 'New Zone'
   - Type : Primary zone
   - Zone name : `computerelectronics.be`
   - Zone file : Create new file
   - Dynamic updates : Allow secure dynamic updates only

3. Création de la zone de recherche inversée :
...
4. Création des enregistrements initiaux :
   - Dans la zone directe, ajoutez :
     ```
     dns1    A    192.168.0.2
     client1 A    192.168.0.101
     client2 A    192.168.0.102
     ```
   - Les enregistrements PTR correspondants seront créés automatiquement

Note : Les enregistrements SRV (_ldap._tcp, _kerberos._tcp, _gc._tcp) seront créés automatiquement lors de la promotion du serveur en contrôleur de domaine.

### 5.2.1.3. Vérification DNS
