...
2. **Machines virtuelles initiales**
   - Un serveur (futur contrôleur de domaine) : `dns1.computerelectronics.be`
   - Un poste client : `ws-compta-01.computerelectronics.be`
   - Possibilité d'ajouter un second DC ultérieurement : `dns2.computerelectronics.be`
...
2. **Zone inverse**
   - Permet **d'obtenir le nom d'hôte** à partir de l'adresse IP
   - Nom : `0.168.192.in-addr.arpa`
   - Gère la résolution inverse pour le réseau 192.168.0.0/24
   - Essentielle pour certains services AD DS
...
![Diagramme des zones DNS](../diagrams/images/structure_reseau_zones.png)
