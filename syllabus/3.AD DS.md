...
On le confond souvent avec **AD DS** (le service de base qui crée et gère la base de données d'AD), mais AD es plus large car il contient:

- **AD DS** **(Active Directory Domain Services)** : **Service principal** gérant l'authentification et l'autorisation des ressources dans le réseau.
- **AD LDS** (Lightweight Directory Services) : Version allégée d'AD DS fonctionnant sans domaine Windows.
- **AD CS** (Certificate Services) : Gestion des certificats numériques et de l'infrastructure à clé publique (PKI).
- **AD RMS** (Rights Management Services) : Protection et contrôle des droits d'accès aux documents.
- **AD FS** (Federation Services) : Authentification unique (SSO) et fédération d'identités entre organisations.
...
## Exemple de fonctionnement d'AD: appel client-serveur

Considerez notre structure de réseau:
...
**Pas les fleches vertes pour le moment**
...
Les fleches vertes rajoutées montrent les **requêtes** DNS permettant la résolution des noms (DNS) nécessaire à chaque étape. On ne voit pas les reponses pour ne plus compliquer le diagramme.

a) Le PC interroge dns1 pour chercher son DC, qui gére l'authentification
b) Une fois authentifié, le PC interroge dns1 pour chercher l'adresse IP du serveur de fichiers. DNS1 ne le connait pas 
c) DNS1 interroge alors DNS2 
DNS2 fournit l'adresse IP du serveur de fichiers. Le PC accède au serveur de fichiers.
...
## 2.1. Structure DNS actuelle du réseau

### 2.1.1. Forêt, arbres, zones DNS
...
`dns1.computerelectronics.be` (10.0.1.10) pour gérer des départements de Comptabilite et RH
`dns2.computerelectronics.be` (10.0.1.11) pour gérer le département de Ventes


## 2.1.2. AD DS et la localisation de la base de données d'AD 
...
- 3 serveurs DNS et 2 serveurs AD 
  * 2 serveurs pour gérer les DNS de Ventes (1 de backup), 1 serveur pour gérer les DNS de Comptabilité et RH
...
## Exercice - Analyse des combinaisons DNS-AD DS
...
## 3. AD DS: Installation et Configuration des Contrôleurs de Domaine

### 3.1 Relations entre DNS et AD DS
...
Nous avions deux serveurs qui fournissent actuellement le service **DNS**, `dns1.computerelectronics.be` (10.0.1.10) et `dns2.computerelectronics.be` (10.0.1.11). 
...
Dans une situation réelle on installera AD aussi sur `dns2.computerelectronics.be` (10.0.1.11), **qui contiendra une replique de la base de données d'AD**. Ou peut-être même sur d'autres serveurs si on les avait!
