...
- La **structure logique** permet d'organiser les ressources **selon les besoins administratifs**
    ...
    - Les équipes sont composées d'utilisateurs (Marie, Lola, Ines, etc.)
...
**La forêt** est le conteneur le plus large.
Voici un example de forêt : 

![Forêt](../diagrams/images/arbre_dns.png)

Nous allons travailler uniquement avec l'entreprise **Computer Electronics**, alors notre forêt aura un seul arbre DNS!

![Forêt](../diagrams/images/structure_reseau_zones.png)

Il y a deux zones dans mais elles font partie du même arbre DNS.
...
Cela se fait grâce au **contrôleur de domaine (DC)**: **un serveur qui héberge une copie de l'annuaire AD DS**

Nous avions deux serveurs DNS, `dns1.computerelectronics.be` et `dns2.computerelectronics.be` et nous allon installer Active Directory sur les deux: **ces serveurs deviendront deux contrôleurs de domaine (DC) de notre forêt**:

- DC1 (dns1) gère les zones DNS pour la comptabilité et les RH
- DC2 (dns2) gère la zone DNS pour les ventes

### 2.2. Flux d'authentification et d'accès aux ressources

#### Authentification et accès aux ressources
...
SECTION CONTENANT L'INSTALLATION D'ACTIVE DIRECTORY
...
1. Zones DNS intégrées à AD DS
   - Les zones DNS sont stockées directement dans la base de données AD DS
...
2. Représentation des objets
   ...
   - Dans AD DS : par son nom distinctif (Distinguished Name - DN)
     Exemple : `CN=pc-compta,OU=Ordinateurs,OU=Comptabilite,DC=computerelectronics,DC=be`
...
Dans notre infrastructure computerelectronics.be :
- Les serveurs DNS (dns1 et dns2) sont installés sur les contrôleurs de domaine
- dns1 gère les zones DNS pour les départements comptabilité et RH
- dns2 gère la zone DNS pour le département ventes
- La réplication AD DS assure la cohérence des données entre tous les DC
...
### Exercice 1.3 - Analyse de l'intégration DNS-AD DS

Analysez le scénario suivant dans l'infrastructure de computerelectronics.be :
Un nouveau poste de travail est ajouté au département comptabilité.

Questions :
1. Quel contrôleur de domaine (DC) va gérer les enregistrements DNS de ce poste ?
2. Comment se fait la réplication de ces informations vers l'autre DC ?
3. Quels sont les enregistrements DNS et AD DS créés ?

<details>
<summary>Solution 1.3</summary>

1. DC1 (dns1) va gérer les enregistrements car :
   - Il est responsable des zones DNS pour le département comptabilité
   - Le poste fait partie du sous-domaine comptabilite.computerelectronics.be

2. Processus de réplication :
   - Les enregistrements sont stockés dans la base AD DS de DC1
   - La réplication AD DS propage automatiquement ces données vers DC2
   - La réplication est plus rapide car intégrée à AD DS

3. Enregistrements créés :
   - Dans DNS :
     ```
     nouveau-pc.comptabilite.computerelectronics.be    A    192.168.x.x
     ```
   - Dans AD DS :
     ```
     CN=nouveau-pc,OU=Ordinateurs,OU=Comptabilite,DC=computerelectronics,DC=be
     ```
</details>
