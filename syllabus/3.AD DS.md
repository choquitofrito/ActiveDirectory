...
L'intégration entre DNS et Active Directory Directory Services (AD DS) est une caractéristique fondamentale de l'infrastructure Windows Server. Cette intégration se manifeste de plusieurs manières :

![Intégration DNS-AD DS](../diagrams/images/integration_dns_ad.png)

Le diagramme ci-dessus illustre notre infrastructure computerelectronics.be :
- DC1 (dns1) gère les zones DNS pour la comptabilité et les RH
- DC2 (dns2) gère la zone DNS pour les ventes
- Les bases de données AD DS stockent les zones DNS intégrées
- La réplication AD DS assure la synchronisation entre les DCs
- Un client effectue d'abord une requête DNS (1) puis s'authentifie (2)

1. Rôle du contrôleur de domaine (Domain Controller - DC)
   - Un contrôleur de domaine est un serveur qui héberge une copie de l'annuaire AD DS
   - Il gère l'authentification et l'autorisation des utilisateurs
   - Par défaut, chaque DC est aussi un serveur DNS pour le domaine
   - Il maintient la synchronisation des données AD DS et DNS

2. Zones DNS intégrées à AD DS
