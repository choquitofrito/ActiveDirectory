...
Après avoir installé et configuré Windows Server, nous allons maintenant nous concentrer sur le service DNS (Domain Name System). Ce service est essentiel pour notre infrastructure car il sera la base de notre futur domaine Active Directory. 
...
Cet exercice montre l'importance du DNS pour :
- Traduire les noms de domaines en adresses IP
- Permettre la communication entre les systèmes
- Gérer les domaines internes et externes
...
Le **DNS** (Domain Name System)** est comme **l'annuaire téléphonique d'Internet**. Son rôle principal est de **transformer les noms de domaine** que nous utilisons (comme `www.computerelectronics.be`) **en adresses IP** (comme `10.2.0.1`). Une adresse IP est **un numéro unique qui identifie un ordinateur sur Internet**.
...
Un espace de noms DNS est organisé de façon hiérarchique, comme un arbre avec :
...
2. **Requêtes itératives**
   - **S'il ne connaît pas le nom** fourni, **il renvoie au client les adresses des serveurs DNS qui gèrent la zone** 
...
2. dns1 n'ayant pas cette information :
   - Contacte un serveur DNS racine pour obtenir l'adresse des serveurs DNS qui gèrent .com
   - Contacte un serveur DNS .com pour obtenir l'adresse des serveurs DNS google.com
   - Contacte un serveur DNS google.com pour obtenir l'IP
...
4. Le serveur DNS .com répond : "Je ne sais pas, mais voici la liste des serveurs DNS qui gèrent google.com"
5. Le client demande l'IP de www.google.com à un des serveurs DNS autoritaires pour google.com
6. Ce serveur DNS autoritaire répond avec l'adresse IP finale de www.google.com
...
- Il peut directement répondre que ws-compta-01.computerelectronics.be → 192.168.0.101
...
2. Un serveur DNS contacte un serveur DNS racine pour trouver le serveur DNS de .com
...
2. Serveur DNS → Serveur DNS racine (.com) :
   - Requête itérative
   - Le serveur DNS accepte une réponse partielle (référence vers les serveurs DNS .com)

3. Serveur DNS → Serveur DNS google.com :
   - Requête itérative
   - Le serveur DNS accepte une réponse directe du serveur DNS autoritaire
</details>
