...
Ex: `ws-compta-01.computerelectronics.be` => `192.168.0.21`
...
- **Nom de domaine (externe)** : `www.computerelectronics.be`
- **Adresse IP (externe)** : `203.0.113.10`
...
Imaginez que vous voulez envoyer une lettre à un ami. Vous connaissez son nom (comme `www.computerelectronics.be`), mais pour que la poste livre la lettre, il faut son adresse complète (comme l'adresse IP `192.168.0.2`). Le DNS fait exactement ça : il traduit les noms en adresses. Si on n'avait pas le DNS, il faudrait que chaque personne connaisse l'adresse IP de chaque site web qu'elle visite!!
...
Notre entreprise a comme nom de domaine `computerelectronics.be` et es composée de plusieurs départements:
