...
1. **Requêtes récursives**
   ...
   - **Il fournit soit l'adresse IP finale, soit une erreur s'il ne connait pas le nom**
   - Le serveur effectue lui-même les requêtes nécessaires
   - Le client ne fait qu'une seule requête

2. **Requêtes itératives**
   - **S'il ne connait pas le nom** fournit, **il renvoie au client les adresses des serveurs qui peuvent la résoudre** 
   - Le **client contactec lui-même ces serveurs** référencés
   - Cela mene à **plusieurs requêtes** de la part du client
...
![Requête DNS récursive](../diagrams/images/dns_requete_recursive.png)
...
![Requête DNS itérative](../diagrams/images/dns_requete_iterative.png)
