...
1. Agissent comme serveurs DNS avec des responsabilités distinctes :
   - `dns1.maxtec.be` (`192.168.0.2`) gère les zones DNS pour les départements comptabilité et RH
   - `dns2.maxtec.be` (`192.168.0.3`) gère la zone DNS pour le département ventes

2. **Maintiennent chacun une copie complète de la base de données AD DS** qui est automatiquement répliquée entre eux

Mais pour notre cours on va installer AD, pour le moment, uniquement sur `dns1.maxtec.be` (`192.168.0.2`) (le seul serveur qu'on a virtualisé).


**Dans notre infrastructure `maxtec.be`, un même ordinateur est représenté** :
   ...
   - Dans **AD DS** : par son **nom distinctif** (Distinguished Name - DN)
     Exemple : `CN=ws-compta-01,OU=Ordinateurs,OU=Comptabilite,DC=maxtec,DC=be`C=be` (on comprendra mieux cette notation plus tard, quand on verra les Unités d'Organisation - OU)
...
1. Localisation des services
   ...
   - Exemple : `_ldap._tcp.maxtec.be` pointe vers les DC
